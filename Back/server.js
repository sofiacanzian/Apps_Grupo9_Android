...
type: String, unique: true, sparse: true },
    ...
    photo: { type: String },
    trainingPreferences: {
        favoriteDisciplines: { type: [String], default: [] },
...
function buildEmptyPreferences() {
    return {
        favoriteDisciplines: [],
        preferredLocations: [],
        preferredTimeRanges: []
...
function toMinutes(timeStr) {
    if (!TIME_RANGE_REGEX.test(timeStr || '')) {
        return null;
    }
...
function normalizePreferenceTimeRanges(ranges) {
    ...
    return ranges
        .map(range => {
            ...
            const label = typeof range.label === 'string' && range.label.trim().length > 0
                ? range.label.trim()
                : `${start}-${end}`;
...
function normalizeTrainingPreferences(rawPreferences = {}) {
    const source = rawPreferences.trainingPreferences || rawPreferences;
    return {
        favoriteDisciplines: sanitizeStringArray(source.favoriteDisciplines),
        preferredLocations: sanitizeStringArray(source.preferredLocations),
        preferredTimeRanges: normalizePreferenceTimeRanges(source.preferredTimeRanges)
    };
}

function formatTrainingPreferences(userPreferences) {
    if (!userPreferences) {
        return buildEmptyPreferences();
    }
    const normalized = normalizeTrainingPreferences(userPreferences);
    return {
        favoriteDisciplines: normalized.favoriteDisciplines,
    ...
    try {
        ...
        res.status(200).json({ 
            ...
            user: {
                ...
                trainingPreferences: formatTrainingPreferences(userObject.trainingPreferences),
...
null,
                trainingPreferences: formatTrainingPreferences(user.trainingPreferences),
...
app.get('/api/preferences/:userId', auth, async (req, res) => {
    ...
    try {
        ...
        res.status(200).json(formatTrainingPreferences(user.trainingPreferences));
    } catch (error) {
        res.status(500).json({ message: 'Error al obtener las preferencias.', error: error.message });
    }
});
...
app.put('/api/preferences/:userId', auth, async (req, res) => {
    ...
    try {
        ...
        const normalized = normalizeTrainingPreferences(req.body);
        user.trainingPreferences = normalized;
        await user.save();

        res.status(200).json(formatTrainingPreferences(user.trainingPreferences));
    } catch (error) {
        res.status(500).json({ message: 'Error al actualizar las preferencias.', error: error.message });
    }
});
...
app.delete('/api/preferences/:userId', auth, async (req, res) => {
    ...
    try {
        ...
        user.trainingPreferences = buildEmptyPreferences();
        await user.save();

        res.status(200).json(formatTrainingPreferences(user.trainingPreferences));
    } catch (error) {
        res.status(500).json({ message: 'Error al restablecer las preferencias.', error: error.message });
    }
});
// =========================================================================
// 🔒 RUTAS DE CLASES 
